...
Репозиторий должен появиться в списке удалённых репозиториев пользователя<sup id="ft3">[3](#usrep)</sup>. Такой репозиторий называется `форк` (от англ. - вилка).
...
Необходимость в обновлении форков может возникать в случае когда основной репозиторий был обновлён, и внесённые изменения необходимо применить также в созданном форке. Общую схему работы с репозитоием можно описать следующей диаграммой:

![](pics/c3.png?raw=true "scheme")
...
Для настройки обновления форка и соответвтующего ему локального репозитория необходимо открыть консоль `git-bash`. Для проверки корректности работы следует пройти в папку локального репозитория и ввести команду `git remote -v`, результатом работы которой должны стать строки

![](pics/a6.png?raw=true "remote")
...
<a id="repman" href="#ft2">2</a>: Репозиторий - это место, где хранятся и поддерживаются какие-либо данные. Используемые в Git репозитории хранят все документы вместе с историей их изменений и служебной информацией. Менеджер удалённых репозиториев - это веб-приложение, которое хранит и предоставляет доступ к репозиториям пользователей.
